...
static int mip_swipe_disable(struct i2c_client* client);
...
static int set_tci_info(struct i2c_client *client)
{
	...
	ts->pdata->tci_info->interrupt_delay_2 = 150;

	return 0;
}

static int set_swipe_info(struct i2c_client *client)
{
	struct mit_data *ts = get_touch_handle_(client);
	TOUCH_INFO_MSG("Setting swipe info data\n");

	//swipe
	ts->pdata->swipe_info->enable_swipe = 1;
	ts->pdata->swipe_info->distance_threshold = 10;
	ts->pdata->swipe_info->ratio_threshold = 200;
	ts->pdata->swipe_info->ratio_check_period = 30;
	ts->pdata->swipe_info->min_time_threshold = 0;
	ts->pdata->swipe_info->max_time_threshold = 3000;
	ts->pdata->swipe_info->interrupt_status = 0;
	ts->pdata->swipe_info->touch_swipe_time = 0;

	TOUCH_INFO_MSG("enable_swipe = %d\n", ts->pdata->swipe_info->enable_swipe);
	TOUCH_INFO_MSG("distance_threshold = %d\n", ts->pdata->swipe_info->distance_threshold);
	TOUCH_INFO_MSG("ratio_threshold = %d\n", ts->pdata->swipe_info->ratio_threshold);
	TOUCH_INFO_MSG("ratio_check_period = %d\n", ts->pdata->swipe_info->ratio_check_period);
	TOUCH_INFO_MSG("min_time_threshold = %d\n", ts->pdata->swipe_info->min_time_threshold);
	TOUCH_INFO_MSG("max_time_threshold = %d\n", ts->pdata->swipe_info->max_time_threshold);
	TOUCH_INFO_MSG("interrupt_status = %d\n", ts->pdata->swipe_info->interrupt_status);
	TOUCH_INFO_MSG("touch_swipe_time = %d\n", ts->pdata->swipe_info->touch_swipe_time);
...
static int mit_probe(struct i2c_client *client, struct touch_platform_data *pdata)
{
	...
	ts->pdata->lpwg_fail_reason = 0;

	set_touch_handle_(client, ts);
	set_tci_info(client);
	set_swipe_info(client);
...
static int mit_touch_event(struct i2c_client *client, struct touch_data *data, u8 *buf, int sz)
{
	...
	for (i = 0; i < sz; i += event_size) {
		...
		if ((tmp[0] & MIP_EVENT_INPUT_SCREEN) == 0) {
			...
			if ((ts->pdata->quick_cover_status == QUICKCOVER_CLOSE
				|| cradle_smart_cover_status() == QUICKCOVER_CLOSE)
				&& ts->pdata->use_quick_window) {
				if (x < ts->pdata->role->quickcover_x1 || x > ts->pdata->role->quickcover_x2
						|| y < ts->pdata->role->quickcover_y1 || y > ts->pdata->role->quickcover_y2) {
					tmp[0] &= ~(1 << 7u) ;
				}
			}
			...
			if((tmp[0] & MIP_EVENT_INPUT_PRESS) == 0) {
				...
				data->curr_data[id].width_minor = touch_minor;
	...
	switch(gesture_code){
		case MIP_LPWG_EVENT_TYPE_FAIL:
			switch (buf[2]) {
				...
				case FAIL_SWIPE_FINGER_RELEASE:   TOUCH_INFO_MSG("LPWG FAIL REASON = Swipe - Finger releas\n");      break;
				case FAIL_SWIPE_MULTIPLE_FINGERS: TOUCH_INFO_MSG("LPWG FAIL REASON = Swipe - Multiple Fingers\n");   break;
				case FAIL_SWIPE_TOO_FAST:         TOUCH_INFO_MSG("LPWG FAIL REASON = Swipe - Too Fast\n");           break;
				case FAIL_SWIPE_TOO_SLOW:         TOUCH_INFO_MSG("LPWG FAIL REASON = Swipe - Too Slow\n");           break;
				case FAIL_SWIPE_UPWARD:           TOUCH_INFO_MSG("LPWG FAIL REASON = Swipe - Upward\n");             break;
				case FAIL_SWIPE_RATIO_EXECESS:    TOUCH_INFO_MSG("LPWG FAIL REASON = Swipe - Ratio Execess\n");      break;
...
static int mit_get_data(struct i2c_client *client, struct touch_data *data)
{
	...
	if(category == 0) {
		...
		alert_type = ts->buf[0];
		TOUCH_INFO_MSG("%s - alert type [%d] sz[%d]\n", __func__, alert_type, sz);
...
void reset_pin_ctrl(int on_off, int delay)
{
	if (ts_data == NULL) {
		...
		gpio_direction_output(ts_data->pdata->reset_pin, on_off);
...
static int mit_power(struct i2c_client* client, int power_ctrl)
{
	...
	if (wakeup_by_swipe_mit300) {
		...
		TOUCH_INFO_MSG("%s : Skip power_control. curr_pwr_state = [%d]\n", __func__, ts->pdata->curr_pwr_state);
		return 0;
	}
...
static int mit_set_active_area(struct mit_data* ts, u8 mode)
{
	...
	if (mode) {
		...
		write_buf[2] = ts->pdata->active_area_x1 & 0xFF;
		write_buf[3] = (ts->pdata->active_area_x1 >> 8) & 0xFF;
		write_buf[4] = ts->pdata->active_area_y1 & 0xFF;
		write_buf[5] = (ts->pdata->active_area_y1 >> 8) & 0xFF;
		write_buf[6] = ts->pdata->active_area_x2 & 0xFF;
		write_buf[7] = (ts->pdata->active_area_x2 >> 8) & 0xFF;
		write_buf[8] = ts->pdata->active_area_y2 & 0xFF;
		write_buf[9] = (ts->pdata->active_area_y2 >> 8) & 0xFF;
...
static int mip_set_wakeup_by_swipe(struct i2c_client* client, u8 value)
{
	u8 wbuf[32];
...
int mip_lpwg_config(struct i2c_client* client)
{
	...
	if (ts->pdata->quick_cover_status == QUICKCOVER_OPEN) {
...
static int mip_swipe_enable(struct i2c_client* client)
{
	...
	wbuf[3] = (ts->pdata->swipe_info->distance_threshold);			// MIP_ADDR_SET_SWIPE_DISTANCE_THRESHOLD
	wbuf[4] = (ts->pdata->swipe_info->ratio_threshold);			// MIP_ADDR_SET_SWIPE_RATIO_THRESHOLD
	wbuf[5] = (ts->pdata->swipe_info->ratio_check_period);			// MIP_ADDR_SET_SWIPE_RATIO_CHECK_PERIOD
	wbuf[6] = (ts->pdata->swipe_info->min_time_threshold) & 0xFF;		// MIP_ADDR_SET_MIN_SWIPE_TIME_THRESHOLD (low byte)
	wbuf[7] = (ts->pdata->swipe_info->min_time_threshold) >>8 & 0xFF;	// MIP_ADDR_SET_MIN_SWIPE_TIME_THRESHOLD (high byte)
	wbuf[8] = (ts->pdata->swipe_info->max_time_threshold) & 0xFF;		// MIP_ADDR_SET_MAX_SWIPE_TIME_THRESHOLD (low byte)
	wbuf[9] = (ts->pdata->swipe_info->max_time_threshold) >>8 & 0xFF;	// MIP_ADDR_SET_MAX_SWIPE_TIME_THRESHOLD (high byte)
	wbuf[10] = (ts->pdata->swipe_info->interrupt_status);			// MIP_ADDR_SET_INTERRUPT_STATUS
	wbuf[11] = (ts->pdata->swipe_info->touch_swipe_time) & 0xFF;		// MIP_ADDR_SET_SWIPE_TOUCH_SWIPE_TIME (low byte)
	wbuf[12] = (ts->pdata->swipe_info->touch_swipe_time) >>8 & 0xFF;	// MIP_ADDR_SET_SWIPE_TOUCH_SWIPE_TIME (high byte)

	TOUCH_INFO_MSG("    swipe_enable(%4d)\n", wbuf[2]);
	TOUCH_INFO_MSG("    distance_threshold(%4d) ratio_threshold(%4d) ratio_check_period(%4d)\n", wbuf[3], wbuf[4], wbuf[5]);
	TOUCH_INFO_MSG("    [swipe time threshold] min(%4d), max(%4d)\n", (wbuf[7]<<8)|wbuf[6], (wbuf[9]<<8)|wbuf[8]);
	TOUCH_INFO_MSG("    interrupt_status(%4d)\n", wbuf[10]);
	TOUCH_INFO_MSG("    touch_swipe_time(%4d)\n", (wbuf[12]<<8)|wbuf[11]);

	if(mip_i2c_write(client, wbuf, 13)){
		TOUCH_ERR_MSG("%s [ERROR] mip_i2c_write\n", __func__);
		return 1;
	}
...
static int mip_swipe_disable(struct i2c_client* client)
{
	...
	wbuf[0] = MIP_R0_LPWG;
	wbuf[1] = MIP_R1_SWIPE_ENABLE;
	wbuf[2] = SWIPE_DISABLE;		// MIP_ADDR_SET_SWIPE_DISABLE

	TOUCH_INFO_MSG("    swipe_disable(%4d)\n", wbuf[2]);
...
static int mit_ic_ctrl(struct i2c_client *client, u32 code, u32 value)
{
	struct mit_data* ts = (struct mit_data *) get_touch_handle_(client);
	...
	switch (code) {
		...
		case IC_CTRL_INFO_SHOW:
			ret = mit_get_ic_info(ts, NULL);
			if (ret != 0) {
				TOUCH_ERR_MSG("%s, fail IC_CTRL_INFO_SHOW\n", __func__);
				return -1;
			}
			if (param) {
				buf = (char *) param->v1;
		...
		case IC_CTRL_SAVE_IC_INFO:
			ret = mit_get_ic_info(ts, NULL);
			if (ret != 0) {
				TOUCH_ERR_MSG("%s, IC_CTRL_SAVE_IC_INFO\n", __func__);
				return -1;
			}
			break;
		...
		case IC_CTRL_ACTIVE_AREA:
			if (param) {
				ret = mit_set_active_area(ts, (u8)param->v1);
				if (ret != 0) {
					TOUCH_ERR_MSG("%s, IC_CTRL_ACTIVE_AREA\n", __func__);
					return -1;
				}
			}
			break;

		case IC_CTRL_WAKEUP_BY_SWIPE:
			if (param) {
				ret = mip_set_wakeup_by_swipe(client, (u8)param->v1);
				if (ret != 0) {
					TOUCH_ERR_MSG("%s, IC_CTRL_WAKEUP_BY_SWIPE\n", __func__);
					return -1;
				}
			}
			break;
...
static ssize_t mit_rawdata_store(struct i2c_client *client, const char *buf)
{
	...
	if (strlen(buf) < 254) {
		...
		TOUCH_INFO_MSG("buf size is more than 255\n");
	}
...
static ssize_t mit_cm_delta_show(struct i2c_client *client, char *buf)
{
	struct mit_data *ts = (struct mit_data *) get_touch_handle_(client);
	int ret = 0;
	ts->pdata->selfdiagnostic_state[SD_CM_DELTA] = 1;	// rawdata

	TOUCH_TRACE_FUNC();
	printk(KERN_ERR "%s\n", __func__);

	ret = mit_get_test_result(client, buf, CM_DELTA_SHOW);
	if (ret < 0) {
		memset(buf, 0, PAGE_SIZE);
		ret = snprintf(buf, PAGE_SIZE, "failed to get raw data\n");
	}
...
static ssize_t mit_cm_delta_store(struct i2c_client *client, const char *buf)
{
	int ret = 0;
	char temp_buf[255] = {0};
	TOUCH_TRACE_FUNC();
	...
	ret = mit_get_test_result(client, temp_buf, CM_DELTA_STORE);

	return ret;
}

static ssize_t mit_cm_jitter_show(struct i2c_client *client, char *buf)
{
	struct mit_data *ts = (struct mit_data *) get_touch_handle_(client);
	int ret = 0;
	ts->pdata->selfdiagnostic_state[SD_CM_JITTER] = 1;	// rawdata
...
static ssize_t mit_cm_jitter_store(struct i2c_client *client, const char *buf)
{
	int ret = 0;
	char temp_buf[255] = {0};
	TOUCH_TRACE_FUNC();
	...
	ret = mit_get_test_result(client, temp_buf, CM_JITTER_STORE);
...
static ssize_t mit_self_diagnostic_show(struct i2c_client *client, char *buf)
{
	...
	memset(buf, 0, PAGE_SIZE);
	ts->pdata->selfdiagnostic_state[SD_CM_DELTA] = 1;	/*CM_DELTA*/
	ret = mit_get_test_result(client, buf, CM_DELTA_SHOW);
	if (ret < 0) {
		TOUCH_ERR_MSG("failed to get cm delta data\n");
		memset(buf, 0, PAGE_SIZE);
		ts->d_max = 0;
		ts->d_min = 0;
...
static ssize_t mit_lpwg_store(struct i2c_client *client, char* buf1, const char *buf2 )
{
	...
	if (buf1 != NULL && !strcmp(buf1,"area")) {
		mit_ic_ctrl(client, IC_CTRL_ACTIVE_AREA, (u32)&set_active_area);
	...
	if (buf1 != NULL && !strcmp(buf1,"update_all")) {
		...
		if (ts->pdata->panel_on == 0 && ts->pdata->lpwg_panel_on == 0) {
			...
			if (!ts->pdata->lpwg_mode && !ts->pdata->lpwg_prox) {
				...
				mip_lpwg_enable_sensing(client, LPWG_ENABLE_SENSING);
...
static ssize_t mit_keyguard_info_store(struct i2c_client *client, const char *buf )
{
	...
	switch(value) {
		case KEYGUARD_RESERVED:
			lockscreen_stat_mit300 = 0;
			TOUCH_INFO_MSG("%s : Lockscreen unlocked, lockscreen_stat = %d\n", __func__, lockscreen_stat_mit300);
...
static int mit_sysfs(struct i2c_client *client, char *buf1, const char *buf2, u32 code)
{
	...
	switch (code) {
		...
		case SYSFS_CMDELTA_SHOW:
			touch_disable(ts->client->irq);
			ret = mit_cm_delta_show(client, buf1);
			touch_enable(ts->client->irq);
			break;
		case SYSFS_CMDELTA_STORE:
			touch_disable(ts->client->irq);
			ret = mit_cm_delta_store(client, buf2);
			touch_enable(ts->client->irq);
			break;
		case SYSFS_CMJITTER_SHOW:
			touch_disable(ts->client->irq);
			ret = mit_cm_jitter_show(client, buf1);
			touch_enable(ts->client->irq);
			break;
		case SYSFS_CMJITTER_STORE:
			touch_disable(ts->client->irq);
			ret = mit_cm_jitter_store(client, buf2);
			touch_enable(ts->client->irq);
			break;
