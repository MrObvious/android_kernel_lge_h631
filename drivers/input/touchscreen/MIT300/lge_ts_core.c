...
#define ts_swipe		(ts_pdata->swipe_info)
...
struct mutex	*mit_thread_lock = NULL;
...
#if 0//defined(CONFIG_LGE_LCD_ESD)
...
int touch_send_wakeup(void *info)
{
	...
	if (ts->pdata->send_lpwg == LPWG_DOUBLE_TAP) {
		send_uevent(lpwg_uevent[0]);
	} else if (ts->pdata->send_lpwg == LPWG_MULTI_TAP) {
		send_uevent(lpwg_uevent[1]);
	} else if (ts->pdata->send_lpwg == LPWG_SWIPE_DOWN) {
...
static void touch_gesture_wakeup_func(struct lge_touch_data *ts)
{
	...
	if (ts->pdata->send_lpwg) {
		if (touch_send_wakeup(ts) < 0) {
			TOUCH_INFO_MSG("%s drop Swipe \n", __func__);
			goto ignore_interrupt;
		}
...
static int touch_parse_dt(struct device *dev, struct touch_platform_data *pdata)
{
	...
	struct touch_dt_map map[] = {
		...
		{ "cm_delta", &limit->cm_delta, DT_U32, 0 },
		{ "cm_jitter", &limit->cm_jitter, DT_U32, 0 },
		...
		{ "use_security_mode", &role->use_security_mode, DT_U8, 0 },
		{ "use_security_all", &role->use_security_all, DT_U8, 0 },
		{ "quickcover_x1", &role->quickcover_x1, DT_U32, 0 },
		{ "quickcover_x2", &role->quickcover_x2, DT_U32, 0 },
		{ "quickcover_y1", &role->quickcover_y1, DT_U32, 0 },
		{ "quickcover_y2", &role->quickcover_y2, DT_U32, 0 },
	...
	for (i = 0; i < TOUCH_PWR_NUM; ++i) {
        ...
        if (lge_get_boot_mode() == LGE_BOOT_MODE_MINIOS) {
                role->use_security_mode = 0;
                role->use_security_all = 0;
        }
...
static int touch_init_platform_data(struct i2c_client *client)
{
	...
	if (client->dev.of_node) {
		...
		ts_swipe = devm_kzalloc(&client->dev,
			sizeof(struct touch_swipe_info), GFP_KERNEL);
		if (!ts_swipe) {
			dev_err(&client->dev, "Failed to allocate memory : lpwg\n");
			return -ENOMEM;
		}
...
static int touch_fb_suspend(struct device *device)
{
	...
	//if (ts->ts_swipe_data.support_swipe > NO_SUPPORT_SWIPE)
		wakeup_by_swipe_mit300 = false;
		touch_drv->ic_ctrl(ts->client, IC_CTRL_WAKEUP_BY_SWIPE, (u32)&wakeup_by_swipe_mit300);
		TOUCH_INFO_MSG("[%s] set reg - WakeupBySwipe, wakeup_by_swipe_mit300 - [%d]\n", __func__, wakeup_by_swipe_mit300);
...
static int touch_fb_resume(struct device *device)
{
	...
	if (ts->pdata->lpwg_mode) {
		...
		if (wakeup_by_swipe_mit300) {
			TOUCH_INFO_MSG("Not working release event\n");
		} else {
			release_all_ts_event(ts);
		}
	} else {
...
static ssize_t cm_delta_show(struct lge_touch_data *ts, char *buf)
{
	ssize_t ret;
	if(ts->pdata->panel_on == POWER_ON || ts->pdata->panel_on == POWER_WAKE || ts->pdata->lpwg_debug_enable != 0) {
		mutex_lock(&ts->thread_lock);
		ret = touch_drv->sysfs(ts->client, buf, 0, SYSFS_CMDELTA_SHOW);
		mutex_unlock(&ts->thread_lock);
	} else {
...
static ssize_t cm_delta_store(struct lge_touch_data *ts, const char *buf, size_t count)
{
	int ret = 0;
	char cmd[NAME_BUFFER_SIZE] = {0};
	if( count > NAME_BUFFER_SIZE )
	...
	if (ts->pdata->panel_on == POWER_ON || ts->pdata->panel_on == POWER_WAKE || ts->pdata->lpwg_debug_enable != 0) {
		...
		mutex_lock(&ts->thread_lock);
		ret = touch_drv->sysfs(ts->client, 0, buf, SYSFS_CMDELTA_STORE);
		mutex_unlock(&ts->thread_lock);
	} else {
		TOUCH_INFO_MSG("If you want to check raw data, please turn on the LCD.\n");
	}
...
static ssize_t cm_jitter_show(struct lge_touch_data *ts, char *buf)
{
	ssize_t ret;
	if(ts->pdata->panel_on == POWER_ON || ts->pdata->panel_on == POWER_WAKE || ts->pdata->lpwg_debug_enable != 0) {
		mutex_lock(&ts->thread_lock);
		ret = touch_drv->sysfs(ts->client, buf, 0, SYSFS_CMJITTER_SHOW);
		mutex_unlock(&ts->thread_lock);
	} else {
		ret = snprintf(buf,PAGE_SIZE,"If you want to check raw data, please turn on the LCD.\n");
		TOUCH_INFO_MSG("If you want to check raw data, please turn on the LCD.\n");
...
static ssize_t cm_jitter_store(struct lge_touch_data *ts, const char *buf, size_t count)
{
	int ret = 0;
	char cmd[NAME_BUFFER_SIZE] = {0};
	if( count > NAME_BUFFER_SIZE )
	...
	if (ts->pdata->panel_on == POWER_ON || ts->pdata->panel_on == POWER_WAKE || ts->pdata->lpwg_debug_enable != 0) {
		...
		mutex_lock(&ts->thread_lock);
		ret = touch_drv->sysfs(ts->client, 0, buf, SYSFS_CMJITTER_STORE);
		mutex_unlock(&ts->thread_lock);
	} else {
		TOUCH_INFO_MSG("If you want to check raw data, please turn on the LCD.\n");
	}
...
static ssize_t lpwg_notify_store(struct lge_touch_data *ts, const char *buf, size_t count)
{
	...
	switch(type) {
		...
		case CMD_LPWG_ACTIVE_AREA:
			ts->pdata->active_area_x1 = cmd[0];
			ts->pdata->active_area_x2 = cmd[1];
			ts->pdata->active_area_y1 = cmd[2];
			ts->pdata->active_area_y2 = cmd[3];
...
static ssize_t use_quick_window_store(struct lge_touch_data *ts, const char *buf, size_t count)
{
	int value;
	sscanf(buf, "%d", &value);

	if( (value == 1) && (ts->pdata->use_quick_window == 0) )
		ts->pdata->use_quick_window = 1;
	else if( (value == 0) && (ts->pdata->use_quick_window == 1) )
		ts->pdata->use_quick_window = 0;
	else
		return count;
...
#if defined(CONFIG_LGE_MODULE_DETECT)
static ssize_t module_id_show(struct lge_touch_data *ts, char *buf)
{
	int ret = 0;
	TOUCH_INFO_MSG("Module id is MIT300\n");
...
static ssize_t quick_cover_status_store(struct lge_touch_data *ts, const char *buf, size_t count)
{
	int value;
	sscanf(buf, "%d", &value);

	if ((value == QUICKCOVER_CLOSE) &&
			(ts->pdata->quick_cover_status == QUICKCOVER_OPEN)) {
		ts->pdata->quick_cover_status = QUICKCOVER_CLOSE;
...
static ssize_t pen_support_show(struct lge_touch_data *ts, char *buf)
{
	int ret = 0;
...
static LGE_TOUCH_ATTR(cmdelta, S_IRUGO | S_IWUSR, cm_delta_show, cm_delta_store);
static LGE_TOUCH_ATTR(cmjitter, S_IRUGO | S_IWUSR, cm_jitter_show, cm_jitter_store);
...
#endif
#if defined(CONFIG_LGE_MODULE_DETECT)
static LGE_TOUCH_ATTR(module_id, S_IRUGO | S_IWUSR, module_id_show, NULL);
#endif
static LGE_TOUCH_ATTR(quick_cover_status, S_IRUGO | S_IWUSR, NULL, quick_cover_status_store);
static LGE_TOUCH_ATTR(pen_support, S_IRUGO | S_IWUSR, pen_support_show, NULL);
static LGE_TOUCH_ATTR(use_quick_window, S_IRUGO | S_IWUSR, NULL, use_quick_window_store);
...
static int touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = touch_drv->ic_ctrl(ts->client, IC_CTRL_INFO_SHOW, 0);
	if (ret < 0) {
		TOUCH_ERR_MSG("FAIL : get touch ic version\n");
		goto err_lge_touch_i2c_connect;
	}
...
*/
	...
	complete_all(&ts->irq_completion);
	mit_thread_lock = &ts->thread_lock;
	is_probed = 1;
	ts->is_probed = true;
...
#ifdef CONFIG_LGE_MODULE_DETECT
	if (get_display_id() != MIT300) {
		ret = -EMLINK;
		TOUCH_INFO_MSG("This panel is not MIT300, get_display_id() = %d\n", get_display_id());
